...
const debug = _debug('nx-project-paths');
...
export default (opts: Options): Plugin => {
  ...
  return {
    ...
    configResolved(config: ResolvedConfig) {
      const tsConfigBaseJson = resolve(
        config.root,
        join(normalizePath(opts.workspaceRoot), 'tsconfig.base.json')
      );
    ...
    async resolveId(id, importer) {
      if (importer && !relativeImportRE.test(id) && !isAbsolute(id)) {
        ...
        const resolved = await resolver(viteResolve, id, importer);
        if (resolved) {
          return resolved;
        }
      }
  ...
  function createResolver(root: string, extensions: string[]): Resolver | null {
    ...
    if (paths) {
      ...
      const resolveWithPaths: Resolver = (
        viteResolve: ViteResolve,
        id: string,
        importer: string
      ) =>
    ...
    const importerExtRE = /\.(vue|svelte|mdx|mjs|tsx|[jt]sx?)$/;
