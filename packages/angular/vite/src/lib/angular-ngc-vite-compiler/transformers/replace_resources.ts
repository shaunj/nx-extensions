...
function isComponentDecorator(
  ...
  if (
    origin &&
    origin.module === '@angular/core' &&
    origin.name === 'Component'
  ) {
    return true;
  }
...
function getDecoratorOrigin(
  ...
  if (symbol && symbol.declarations && symbol.declarations.length > 0) {
    ...
    if (ts.isImportSpecifier(declaration)) {
      ...
      module = (
        declaration.parent.parent.parent.moduleSpecifier as ts.Identifier
      ).text;
    } else if (ts.isNamespaceImport(declaration)) {
      // Use the name from the decorator namespace property access
      module = (declaration.parent.parent.moduleSpecifier as ts.Identifier)
        .text;
